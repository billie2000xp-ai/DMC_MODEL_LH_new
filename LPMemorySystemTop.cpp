...
LPMemorySystemTop::LPMemorySystemTop(unsigned hhaId, string IniFilePath, string LogPath,
    ...
    PDU_HIT_LEVEL = cfg->getNumber("PDU_HIT_LEVEL");
    PDU_RD_DEPTH = cfg->getNumber("PDU_RD_DEPTH");
    PDU_WR_DEPTH = cfg->getNumber("PDU_WR_DEPTH");
    ...
    IECC_PRI_FOLLOW = cfg->getBool("IECC_PRI_FOLLOW");
    ...
    GET_PARAM(PDU_HIT_LEVEL, "PDU_HIT_LEVEL", getUint);
    GET_PARAM(PDU_RD_DEPTH, "PDU_RD_DEPTH", getUint);
    GET_PARAM(PDU_WR_DEPTH, "PDU_WR_DEPTH", getUint);
    ...
    GET_PARAM(IECC_PRI_FOLLOW, "IECC_PRI_FOLLOW", getBool);
...
//    }
    ...
    if (!top_wresp_fifo.empty()) {
        for (size_t i = 0; i < top_wresp_fifo.size(); i++) {
            auto &pkt = top_wresp_fifo[i];
            ...
            if (accepted) {
                top_wresp_fifo.erase(top_wresp_fifo.begin() + i);
                break;
            }
        }
    }
